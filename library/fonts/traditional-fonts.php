<?php
class PadmaTraditionalFonts extends PadmaWebFontProvider {
	...
	public function query_fonts($sortby) {
		...
		foreach ( PadmaFonts::get_fonts() as $font_id => $font_name )
			$fonts[] = array(
				'id' => $font_id,
				'name' => $font_name,
				'stack' => PadmaFonts::get_stack($font_id) 
			);
...
class PadmaFonts {
	...
	public static function get_fonts() {
		...
		return apply_filters('padma_fonts', self::$fonts); 
	...
	public static function get_stack($font_id) {
		...
		return padma_get($font_id, apply_filters('padma_fonts_stacks', self::$font_stacks));
