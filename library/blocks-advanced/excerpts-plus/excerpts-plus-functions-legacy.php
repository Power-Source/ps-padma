    ...
    static function parse_meta($block, $settings, $meta)
    {
      ...
      if (strpos($meta, '%comments%') !== false || strpos($meta, '%comments_no_link%') !== false) {
        if ((int)get_comments_number($post->ID) === 0) {
          $comments_format = stripslashes(PadmaBlockAPI::get_setting($block, 'comment-format-0', $settings[ 'ep-text-comments-nil' ]));
        } elseif ((int)get_comments_number($post->ID) == 1) {
          $comments_format = stripslashes(PadmaBlockAPI::get_setting($block, 'comment-format-1', $settings[ 'ep-text-comments-single' ]));
        } elseif ((int)get_comments_number($post->ID) > 1) {
          $comments_format = stripslashes(PadmaBlockAPI::get_setting($block, 'comment-format', $settings[ 'ep-text-comments-multiple' ]));
        }
      ...
      if (strpos($meta, '%respond%') !== false) {
        $respond_format = stripslashes(PadmaBlockAPI::get_setting($block, 'respond-format', $settings[ 'ep-text-comments-new' ]));
...
// Functions ourside of the class
  ...
  function ep_extra_ve_css()
  {
    ...
    wp_enqueue_style('Padma-ep-ve-css', EP_BLOCK_URL . '/css/ep2_visual_editor.css');
  }
...
// in the future, run this in wp_activate
    ...
    if ($needs_upgrade) {
      ...
      PadmaBlocksData::update_block($block[ 'layout' ], $block[ 'id' ], $block);
    }
  }
        ...
        if (!empty($ep_custom_fields_name_val)) {
          ...
          if ($settings_cf[ 'ep-custom-fields-name-url' ] != 'none') {
            ...
            if (!empty($settings_cf[ 'ep-custom-fields-is-image' ]) && $settings_cf[ 'ep-custom-fields-is-image' ] !='false') {
              ...
              $ep_custom .= date_i18n(get_option('date_format'), str_replace(',', ' ', strtotime($ep_custom_fields_name_val)));
            } else {
